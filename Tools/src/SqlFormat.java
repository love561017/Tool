		...
		for (int b = 0; b < j; b++) {
			...
			if (line.indexOf("@") > 0) {
				...
				if (tempLineToUpper.indexOf("FROM") >= 0) {
					...
					String table = StringUtils.trim((temp.length == 0 ? tempLineToUpper.replace("JOIN", "") : temp[1]).replace("@", ""));
					newDesc = TABLE_NAMES_MAP.get(table.split(" ")[0]);
					asName =  " (" + table.split(" ")[1].toLowerCase() + ")";
				}else if (tempLineToUpper.indexOf("AS") > 0) {
					String[] temp = tempLineToUpper.split("AS");
					String column = StringUtils.trim((temp.length == 0 ? tempLineToUpper.replace("AS", "") : temp[1]).replace("@", ""));
					...
					newDesc = null == TABLE_COLUMNS_MAP.get(column) ? TABLE_NAMES_MAP.get(column) : TABLE_COLUMNS_MAP.get(column);
				}
				if (null != newDesc) {
